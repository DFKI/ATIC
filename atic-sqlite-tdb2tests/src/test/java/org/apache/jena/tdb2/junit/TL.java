...
public class TL {
    ...
    public static void expel(DatasetGraph dataset) {
        //atic
        if(dataset instanceof SqliteAticDatasetGraph) {
            dataset.close();
        } else {
    ...
    static {
        //simulate auto-commit: an add() directly writes to database
        SqliteAticGraph.setDefaultBufferAndBatchSize(1);
    }
    
    private static Dataset createTestDataset() {
        return AticFactory.createTxnAdminDataset();
    }
    ...
    public static DatasetGraph createTestDatasetGraphMem() {
        return AticFactory.createTxnAdminDataset().asDatasetGraph();
    }
}
