...
import de.l3s.tools.documentgenerator.DocumentGenerator;
import de.l3s.tools.documentgenerator.Topic;
import de.l3s.tools.namegenerator.NameGenerator;
import java.io.File;
...
public class Generator {
  ...
  /** first name */
  static final int CS_P_FIRSTNAME = CS_P_NAME + 1;
  /** surname */
  static final int CS_P_SURNAME = CS_P_FIRSTNAME + 1;
  /** full name */
  static final int CS_P_FULLNAME = CS_P_SURNAME + 1;
  /** takesCourse */
  static final int CS_P_TAKECOURSE = CS_P_FULLNAME + 1;
  /** teacherOf */
  ...
  /** publicationText */
  static final int CS_P_PUBLICATIONTEXT = CS_P_PUBLICATIONAUTHOR + 1;
  /** headOf */
  static final int CS_P_HEADOF = CS_P_PUBLICATIONTEXT + 1;
  /** teachingAssistantOf */
  ...
  /** size of publication texts */
  private static final int PUBLICATION_TEXT_SIZE = 1000;
  ...
  /** generate full names for persons */
  private NameGenerator nameGenerator_;
  /** generate abstracts for publications */
  private DocumentGenerator docGenerator_;
  /** topic of the current department (for docGenerator_) */
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      while (i < args.length) {
        ...
        else if (arg.equals("-anonymousnames")) {
        	names = 2;
        }
    ...
    new Generator().start(univNum, startIndex, seed, daml, names, docs, ontology);
  }
  */
   ...
   */
  public void start(int univNum, int startIndex, int seed,
                    int names, boolean docs, String ontology, Writer writer) {
    ...
    if(names > 0) {
    	nameGenerator_ = new NameGenerator();
    	nameGenerator_.setSeed(0);
    	if(names == 2)
    		nameGenerator_.setRandomizedNames(true);
    }
    
    if(docs) {
    	try {
        	docGenerator_ = new DocumentGenerator();
		} catch (IOException e) {
			System.err.println("Could not initiate the document generator!\n" + e.getMessage());
		}
    }
    ...
    _generate(logFilename);
    System.out.println("See "+ logFilename +" for more details.");
  }
  ...
  private void _generate(String logFilename) {
    System.out.println("Started...");
    try {
        
        log_ = new PrintStream(
  ...
  private void _generateDept(int univIndex, int index) {
    ...
    for (int i = 0; i < UNDER_COURSE_NUM; i++) {
      remainingUnderCourses_.add(i); //was: new Integer(i)
    }
    for (int i = 0; i < GRAD_COURSE_NUM; i++) {
      remainingGradCourses_.add(i); //was: new Integer(i)
    }
    publications_.clear();
    for (int i = 0; i < CLASS_NUM; i++) {
    ...
    //pick a random topic for this department
    if(docGenerator_ != null) {
    	topic_ = docGenerator_.randomTopic();
    }
  ...
  private void _generateAFaculty_a(int type, int index) {
    ...
    if(nameGenerator_ != null) {
    	// generate first name, surname and full name
    	String firstname = nameGenerator_.nextFirstname();
    	String surname = nameGenerator_.nextSurname();
    	
    	writer_.addProperty(CS_P_FIRSTNAME, firstname, false);
    	writer_.addProperty(CS_P_SURNAME, surname, false);
    	writer_.addProperty(CS_P_FULLNAME, firstname + " " + surname, false);
    }
  ...
  private void _generateAPublication(PublicationInfo publication) {
    ...
    if(docGenerator_ != null) {
    	Topic[] topics = new Topic[publication.authors.size() + 1];
    	topics[0] = topic_;
    	
    	// collect the topics of all authors
    	for(int i=0; i<publication.authors.size(); i++) {
    		String author = (String)publication.authors.get(i);
    		
    		// should produce a uniformly distributed random number [0..1],
    		// which is always the same for the same author
        	double rand = (double)author.hashCode() / (double)Integer.MAX_VALUE;
        	
        	// now pick a topic related to the department topic for this author based on the rand variable
        	topics[i+1] = topic_.getCorrelatedTopic(rand);
    	}

    	// generate the publication text
    	String text = docGenerator_.nextDocument(PUBLICATION_TEXT_SIZE, topics);

    	// flush the text
        writer_.addProperty(CS_P_PUBLICATIONTEXT, text, false);
    }
  ...
  private void _generateComments() {
    ...
    log_.println(comment);
    for (int i = 0; i < CLASS_NUM; i++) {
      comment = CLASS_TOKEN[i] + " " + instances_[i].logNum + " " + instances_[i].logTotal;
      log_.println(comment);
