...
public class TestDriver {
    ...
    protected static Logger logger = Logger.getLogger(TestDriver.class.getName());
    ...
    public void run(Consumer<String> queryConsumer) {
        int qmsPerPeriod = TestDriverDefaultValues.qmsPerPeriod;
        int qmsCounter = 0;
        int periodCounter = 0;
        double periodRuntime = 0;
        ...
        for (int nrRun = -warmups; nrRun < nrRuns; nrRun++) {
            ...
            while (queryMix.hasNext()) {
                ...
                if (ignoreQueries[next.getNr() - 1]) {
                    ...
                    queryConsumer.accept(next.getQueryString());
    ...
    public void runMix(Function<String, Integer> query2resultCount) {
        int nrRun = -1;
        while (queryMix.hasNext()) {
            Query next = queryMix.getNext();
            ...
            if (ignoreQueries[next.getNr() - 1]) {
                ...
                int resultCount = query2resultCount.apply(next.getQueryString());
                queryMix.setCurrent(resultCount, -1.0);
            }
        }
        queryMix.finishRun();
    }
    ...
    public static String getSparqlUpdateQueryParameter() {
        return sparqlUpdateQueryParameter;
    }
    ...
    public boolean[] getIgnoreQueries() {
        return ignoreQueries;
    }

    public boolean isDoSQL() {
        return doSQL;
    }
